...
#cdst_btot=np.concatenate((rbygsm7, sta_bt7))
...
ax6.xaxis.set_major_formatter(myformat)
plt.xlim([np.ceil(rbtime_num)[0]+realtime_plot_leftadd,rbtime_num[-1]+realtime_plot_timeadd])
plt.ylim([np.nanmin(dst_burton)-50,50])
plt.xticks(fontsize=fsize)
plt.yticks(fontsize=fsize)
...
if verification_mode > 0:
  ...
  [timenowb, sta_ptime, sta_vr, sta_btime, sta_btot, sta_br,sta_bt, sta_bn, rbtime_num, rbtot, rbzgsm, rptime_num, rpv, rpn, lrdst_time, lrdst, lcdst_time, ldst_burton, ldst_obrien]=pickle.load(open(verify_filename,'rb') )  
...
pickle.dump([timenowb, sta_ptime, sta_vr, sta_btime, sta_btot, sta_br,sta_bt, sta_bn, rbtime_num, rbtot, rbzgsm, rptime_num, rpv, rpn, rdst_time, rdst, cdst_time, dst_burton, dst_obrien], open(filename_save, "wb" ) )
#load with
